...
<div  class="login-container"  >
    <el-form ref="form" :model="form"    :rules="rules" class="login-page">
      ...
      <el-form-item prop="role">
        ...
        <el-radio v-model="form.role" label="1">作者</el-radio>
        <el-radio v-model="form.role" label="0">管理员</el-radio>
      </el-form-item>
      <el-form-item prop="authorize" v-if="form.role==0">
      ...
      <el-form-item prop="authorize" v-if="form.role==1">
        <el-input v-model="form.authorize" placeholder="请输入作者邀请码：zuozhe" clearable show-password>
          <template #prefix>
            <el-icon class="el-input__icon"><Lock /></el-icon>
          </template>
        </el-input>
    ...
    register(){
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if(this.form.role == 0 && this.form.authorize != "2236")
          {
            ElMessage.error("请输入正确的注册码")
            return
          }
          if(this.form.role == 1 && this.form.authorize != "zuozhe")
          {
            ElMessage.error("请输入正确的作者邀请码")
            return
          }
